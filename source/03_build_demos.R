...
get_acs(year = 2015, table = "B19001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
...
get_acs(year = 2015, variable = "B19013_001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
  select(GEOID, B19013_001E) %>%
  rename(median_inc = B19013_001E) -> median_inc

# median value of owner occupied housing ####
get_acs(year = 2015, variable = "B25077_001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
  select(GEOID, B25077_001E) %>%
  rename(owner_occ_value = B25077_001E) -> owner_occ_value

# proportion below poverty line ####
get_acs(year = 2015, table = "B17001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
  select(GEOID, B17001_001E, B17001_002E) %>%
  mutate(poverty_pct = B17001_002E/B17001_001E*100) %>%
  select(GEOID, poverty_pct) -> poverty_pct

# proportion vacant ####
get_acs(year = 2015, variable = "B25001_001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
  select(GEOID, B25001_001E) -> total_housing_units

get_acs(year = 2015, variable = "B25004_001", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
...
# owner occupied ####
get_acs(year = 2015, table = "B25008", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
...
get_acs(year = 2015, table = "B23025", geography = "tract", state = 29, county = c(189, 510), output = "wide") %>%
...
# write ####
write_csv(demographics, "data/STL_DEMOGRAPHY_Current.csv")
